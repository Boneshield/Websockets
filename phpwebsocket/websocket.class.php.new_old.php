...
class WebSocket{
  ...
  function dohandshake($user,$buffer){
    ...
    $this->log($buffer);
    list($resource,$host,$origin,$key1,$key2,$l8b,$version,$newkey) = $this->getheaders($buffer);
    $this->log("Handshaking...");
    //$port = explode(":",$host);
    //$port = $port[1];
    //$this->log($origin."\r\n".$host);

    if($version && $version>=7) {  // New Protocol Version hybie-07
      $keyaccept = $this->calcNewKey($newkey);
      $oldkey = '';
      $upgrade = "HTTP/1.1 101 Switching Protocols\r\n";
    } else { // Old Protocol Version hixie-76
      $keyaccept = '';
      $oldkey = $this->calcKey($key1,$key2,$l8b);
    ...
    $upgrade .= 
                ...
                // New Protocol Version hybie-07
                if($keyaccept) {
                  $upgrade .= "Sec-WebSocket-Accept: ".$keyaccept."\r\n";
                  // $upgrade .= "Sec-WebSocket-Protocol: chat\r\n";
                }

                // Old Protocol Version hixie-76
                if($oldkey) {
                  $upgrade .= "\r\n".$oldkey."\r\n";
                }
  ...
  // New Protocol Version hybie-07
  function calcNewKey($key) {
    $newkey = '';
    $key = sha1($key.'258EAFA5-E914-47DA-95CA-C5AB0DC85B11');
    for ($i = 0; $i < strlen($key); $i+=2) {
      $newkey .= chr(intval($key[$i].$key[$i+1], 16));
    }
  ...
  function getheaders($req){
    $r=$h=$o=$version=$newkey=$sk1=$sk2=null;
    echo $req; // Debug headers
    if(preg_match("/GET (.*) HTTP/"               ,$req,$match)){ $r=$match[1]; }
    if(preg_match("/Host: (.*)\r\n/"              ,$req,$match)){ $h=$match[1]; }
    if(preg_match("/Origin: (.*)\r\n/"            ,$req,$match)){ $o=$match[1]; }
    if(preg_match("/Sec-WebSocket-Version: (.*)\r\n/" ,$req,$match)){ $version=$match[1]; }
    if(preg_match("/Sec-WebSocket-Key: (.*)\r\n/" ,$req,$match)){ $newkey=$match[1]; }
    ...
    return array($r,$h,$o,$sk1,$sk2,$l8b,$version,$newkey);
  }
